...
export class ShadowDrawer extends Component {
    ...
    @property({
        tooltip: 'color of shadow render'
    })
    ...
    @property({ 
        ...
        min: 0, 
        max: 255,
        tooltip: 'opacity of shadow render' 
    })
    public shadowOpacity: number = 255;
    @property({
        tooltip: 'angle of shadow casting'
    })
    public shadowAngle: number = 0;
    @property({
        tooltip: 'length of shadow render'
    })
    public shadowLength: number = 10;
    @property({
        tooltip: 'degree of blurring shadow edges'
    })
    public shadowBlurWidth: number = 10;
    @property({ 
        type: EffectAsset,
        tooltip: 'effect for shadow graphics fill' 
    })
    public fillEffect: EffectAsset = null;
    @property({ 
        type: EffectAsset,
        tooltip: 'effect for shadow graphics stroke' 
    })
    public strokeEffect: EffectAsset = null;
    @property({ 
        type: EffectAsset,
        tooltip: 'effect for shadow render texture' 
    })
    ...
    private _renderMaterial: Material = null; 
    ...
    _createRenderNode() {
        ...
        renderNode.layer = Layers.Enum.DEFAULT;
    ...
    _setShadowMaterial(renderer: Sprite | Graphics, effectAsset: EffectAsset) {
        if (renderer && effectAsset) {
            ...
            this._renderMaterial = renderer.sharedMaterial;
            
            this.setShadowColor(this.shadowColor);
            this.setShadowOpacity(this.shadowOpacity);
    ...
    setShadowColor(color: Color) {
        ...
        if (this._renderMaterial.getProperty('shadowColor') !== undefined) {
            this._renderMaterial.setProperty('shadowColor', color);
        }
    }
    ...
    setShadowOpacity(opacity: number) {
        ...
        if (this._renderMaterial.getProperty('shadowOpacity') !== undefined) {
            this._renderMaterial.setProperty('shadowOpacity', opacity / 255);
        }
    }

    setShadowAngle(angle: number, redraw = false) {
    ...
    setShadowLength(length: number, redraw = false) {
    ...
    setShadowBlurWidth(width: number, redraw = false) {
        this.shadowBlurWidth = width;
        this._strokeGraphics.lineWidth = width;
        if (redraw) {
            this._drawShadow();
        }
    }
